...
# MAGIC
# MAGIC <img src="files/shared_uploads/mark.lee@databricks.com/46_2016_Calicivirus_Poster.png"/>
...
# MAGIC <img src="https://raw.githubusercontent.com/bluerider/distributed_qsar/master/6cox_docked_4.png" width = "33%">
# MAGIC <img src="https://raw.githubusercontent.com/bluerider/distributed_qsar/master/6cox_docked_3.png" width = "33%">
# MAGIC <img src="https://raw.githubusercontent.com/bluerider/distributed_qsar/master/6cox_docked_3.png" width = "33%">
...
# DBTITLE 1,Install Descriptastorus and Chemprop
...
# MAGIC %sh
# MAGIC
# MAGIC sudo apt install --assume-yes libglu1-mesa-dev mesa-common-dev              ## install libgl headers
...
# MAGIC <img src="https://raw.githubusercontent.com/bluerider/distributed_qsar/master/QSAR%20Architecture_ingest.png">
...
# MAGIC
# MAGIC ## Architecture Step
# MAGIC <img src="https://raw.githubusercontent.com/bluerider/distributed_qsar/master/QSAR%20Architecture_clean.png">
...
# MAGIC
# MAGIC ## Architecture Step
# MAGIC <img src="https://raw.githubusercontent.com/bluerider/distributed_qsar/master/QSAR%20Architecture_vina.png">
...
# MAGIC %md
# MAGIC ## Architecture Step
# MAGIC <img src="https://raw.githubusercontent.com/bluerider/distributed_qsar/master/QSAR%20Architecture_mlflow.png">
...
# MAGIC """
# MAGIC
# MAGIC mlflow.set_experiment("/Users/mark.lee@databricks.com/MoleculeML")
...
# MAGIC                .withColumn("F1", 1 / col("Accuracy"))\
# MAGIC                .select("Max Depth", "F1")
...
# MAGIC """
# MAGIC
# MAGIC _model = mlflow.spark.load_model("dbfs:/databricks/mlflow-tracking/2737876/b9baa8b24cfc4717b787e56260829613/artifacts/Decision_tree_3")
# MAGIC
...
# MAGIC %md
# MAGIC
# MAGIC # Using Chemprop

# COMMAND ----------

# MAGIC %md
# MAGIC
# MAGIC ## Setup a dataset

# COMMAND ----------

# DBTITLE 1,Visualize the Vina Results Dataset
# MAGIC %sql
# MAGIC
# MAGIC --
# MAGIC -- Get the results of a vina docking study
# MAGIC --
# MAGIC
# MAGIC SELECT SMILES as smiles_columns,
# MAGIC        vina_results.docked_conformer_vina_results.binding_affinity
# MAGIC FROM VINA_RESULTS;
...
# MAGIC
# MAGIC import os, subprocess
# MAGIC from tempfile import TemporaryDirectory
# MAGIC from typing import List, Tuple, Union
# MAGIC from pyspark.sql import DataFrame
# MAGIC
# MAGIC """
# MAGIC Create a chemprop training function
# MAGIC """
# MAGIC
# MAGIC def run_chemprop_train(data : DataFrame) -> List[List[str]]:
# MAGIC   """
# MAGIC   Test run the chemprop
# MAGIC   """
# MAGIC   
# MAGIC   # create a temporary directory
# MAGIC   temp_dir : TemporaryDirectory = TemporaryDirectory()
# MAGIC   
# MAGIC   # write to a temporary file
# MAGIC   train_file : str = f"{temp_dir.name}/train.csv"
# MAGIC   
# MAGIC   data.write\
# MAGIC       .format("csv")\
# MAGIC       .save(train_file)
# MAGIC   
# MAGIC   # train model quantification
# MAGIC   results = subprocess.run(["chemprop_train", "--data_path", train_file, "--dataset_type", "regression", "--save_dir", temp_dir.name], capture_output = True)
# MAGIC   
# MAGIC   # retrieve models from temp_dir.name
# MAGIC #   models = os.listdir(f"{temp_dir.name}/checkpoints")
# MAGIC   
# MAGIC   # read one of them into a binary string
# MAGIC #   with open(models[0], 'rb') as f:
# MAGIC #       model = f.read()
# MAGIC       
# MAGIC   # clean up
# MAGIC #   temp_dir.cleanup()
# MAGIC   
...
def chemprop_training_run(hyperparameters : Dict,
                          training_data : DataFrame,
                          test_data : DataFrame) -> Tuple[int, float]:
  with mlflow.start_run() as run:
    # log some parameters
    mlflow.log_param("hyperparameters", str(hyperparameters))
    
    # train the model
    _results = run_chemprop_train(training_data)
...
#     mlflow.log_param("Feature Importances", _model.featureImportances)
    ...
    # Log the model
#     mlflow.spark.log_model(_model, f"CHEMPROP_{p_max_depth}")
    return _results
#     return (p_max_depth, _accuracy)

# COMMAND ----------

chemprop_training_run({}, _training_df.drop("features"), _test_df.drop("features"))
