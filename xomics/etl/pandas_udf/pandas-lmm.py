...
pop = pd.read_csv('ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/integrated_call_samples_v2.20130502.ALL.ped', sep='\t')
pop.head(5)
...
"""
Read in 1000genomes phase 3 chr 22 with multiallelic sites split to biallelic.
...
df = spark.read.format("com.databricks.vcf").option("splitToBiallelic", True)\
  .load("/databricks-datasets/genomics/1kg-vcfs/*chr22*.vcf.gz")\
  .crossJoin(spark.read.format("parquet").load("/hls/phenotypes.normalized"))\
