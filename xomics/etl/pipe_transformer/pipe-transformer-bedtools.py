...
bed = spark.createDataFrame([(22, 16050000, 16060000), 
                            ...
                            ("#chrom", "start", "end"))
bed.toPandas().to_csv("/dbfs/tmp/william.brandler@databricks.com/chr22.bed", sep="\t", index=False)
...
/mnt/dbnucleus/lib/bedtools2/bin/bedtools intersect -seed 42 -a - -b /dbfs/tmp/william.brandler@databricks.com/chr22.bed -header -wa
...
vcf_output_path = "dbfs:/tmp/william.brandler@databricks.com/test.vcf"
(
  df_intersect
  .write
  .format("bigvcf")
...
# MAGIC %sh
# MAGIC cat /dbfs/tmp/william.brandler@databricks.com/test.vcf

# COMMAND ----------

dbutils.fs.rm("dbfs:/tmp/william.brandler@databricks.com/", True)
